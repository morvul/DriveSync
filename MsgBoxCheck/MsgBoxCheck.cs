...
namespace MsgBoxCheck
{
	public class MessageBox
	{
		...
		public DialogResult Show(string strCheck, string strText, string strTitle, MessageBoxButtons buttons, MessageBoxIcon icon, ref bool showAgain)
		{
			m_strCheck = strCheck;
			m_bCheck = showAgain;
			m_cbt.Install();
			DialogResult dr = System.Windows.Forms.MessageBox.Show(strText, strTitle, buttons, icon);
			showAgain = m_bCheck;
			m_cbt.Uninstall();
			return dr;
		}

		public DialogResult Show(string strCheck, string strText, string strTitle, MessageBoxButtons buttons, ref bool showAgain)
		{
			return Show(strCheck, strText, strTitle, buttons, MessageBoxIcon.None, ref showAgain);
		}

		public DialogResult Show(string strCheck, string strText, string strTitle, ref bool showAgain)
		{
			return Show(strCheck, strText, strTitle, MessageBoxButtons.OK, MessageBoxIcon.None, ref showAgain);
		}

		public DialogResult Show(string strCheck, string strText, ref bool showAgain)
		{
			return Show(strCheck, strText, "", MessageBoxButtons.OK, MessageBoxIcon.None, ref showAgain);
		}
		...
		private void WndActivated(object sender, CbtEventArgs e)
		{
			...
			MoveWindow(m_hwnd,rc.left,rc.top,365,rc.bottom-rc.top + (int)fCur.GetHeight()*2,true);

			m_hwndBtn = CreateWindowEx(0, "button", m_strCheck, BS_AUTOCHECKBOX|WS_CHILD|WS_VISIBLE|WS_TABSTOP, 
				x, y , 365, (int)fCur.GetHeight(),
