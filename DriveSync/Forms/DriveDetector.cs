...
namespace DriveSync
{
	...
	class DriveDetector : IDisposable
	{
		...
		public static void EjectDrive(char driveLetter)
		{
			...
			if ((long)handle == -1)
			{
				MessageBox.Show("Не удалось извлечь диск с указанным имененем (" + driveLetter+")", "Ошибка извлечения диска", MessageBoxButtons.OK, MessageBoxIcon.Warning);
				return;
			}
					...
					case DBT_DEVICEREMOVECOMPLETE:
						...
						if (devType == DBT_DEVTYP_VOLUME)
						{
							...
							if (devType == DBT_DEVTYP_VOLUME)
							{
								...
								// UNDONE    we could unregister the notify handle here if we knew it is the
